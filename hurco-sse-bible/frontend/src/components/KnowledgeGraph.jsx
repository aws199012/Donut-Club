...
import { CATEGORY_COLORS, CATEGORY_LABELS, CATEGORY_SINGULAR } from './entityCategories.js';
...
export default function KnowledgeGraph({ graph, local, onOpenDocument, onOpenTicket }) {
  ...
  // graph.forceRender skips the sparse fallback — set by the Entity Explorer, whose
  // scoped 1-hop graphs are legitimately small.
  const isSparse =
    !graph.forceRender &&
    graph.meta.entityCount + (graph.meta.eventCount || 0) < MIN_ENTITIES_FOR_GRAPH;
